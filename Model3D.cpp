...
namespace gps {
	...
	GLuint Model3D::ReadTextureFromFile(const char* file_name) {
		...
		// --- MODIFICAREA 1: Formatul ---
		// Schimba GL_SRGB cu GL_RGBA.
		// GL_SRGB risca sa piarda canalul Alpha sau sa faca blending gresit.
		glTexImage2D(
			GL_TEXTURE_2D,
			0,
			GL_RGBA, // <--- AICI ERA GL_SRGB. PUNE GL_RGBA
			x,
			y,
			0,
			GL_RGBA,
		...
		// --- MODIFICAREA 2: Wrapping ---
		// Schimba GL_REPEAT cu GL_CLAMP_TO_EDGE.
		// Asta repara dungile acelea urate de pe marginile copacilor.
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE); // <--- AICI
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE); // <--- AICI
